export function generateInsights(actualSales: number[], predictedSales: number[]): string[] {
  ...
  if (growthRate > 0) {
    insights.push(
      `Sales are predicted to grow by ${growthRate.toFixed(2)}% over the next ${predictedSales.length} days. Consider increasing inventory to meet potential demand.`,
    )
  } else if (growthRate < 0) {
    insights.push(
      `Sales are predicted to decline by ${Math.abs(growthRate).toFixed(2)}% over the next ${predictedSales.length} days. Consider adjusting inventory and exploring new marketing strategies.`,
    )
  } else {
    insights.push(
      `Sales are predicted to remain stable over the next ${predictedSales.length} days. Maintain current inventory levels and continue monitoring market trends.`,
    )
  }
